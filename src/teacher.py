...
class MTConfig(PretrainedConfig):
    ...
    def __init__(self, backbone="microsoft/mdeberta-v3-base", num_labels=8, gamma_focal=2.0, **kw):
        super().__init__(**kw)
...
class ModelFactory:
    ...
    def create_teacher(num_labels: int) -> MTModel:
        cfg = MTConfig(backbone=CFG.teacher_model, num_labels=num_labels, gamma_focal=CFG.gamma_focal)
...
class MetricStrategy:
    def __call__(self, eval_pred):
        ...
        return {"macro_f1": float(f1_score(labels, y_pred, average="macro", zero_division=0))}
