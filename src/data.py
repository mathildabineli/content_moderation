...
def split_dataset() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, List[str], str]:
    ...
    s1 = MultilabelStratifiedShuffleSplit(n_splits=1, test_size=0.20, random_state=CFG.seed)
    trp_idx, te_idx = next(s1.split(df[[text_col]], df[labels]))
    train_pool, test_df = df.iloc[trp_idx], df.iloc[te_idx]

    s2 = MultilabelStratifiedShuffleSplit(n_splits=1, test_size=0.10, random_state=CFG.seed+1)
...
def build_hf_dataset(tokenizer, df: pd.DataFrame, text_col: str, labels: List[str]):
    enc = tokenizer(df[text_col].tolist(), return_tensors="np", truncation=True,
                    padding=True, max_length=CFG.max_len)
